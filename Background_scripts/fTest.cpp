...
int main(int argc, char* argv[]){
   ...
   //namingMap.insert(pair<string,string>("Chebychev","cheb")); // Chebyshev??
   ...
   for (int cat=startingCategory; cat<ncats; cat++){
      ...
      for (vector<string>::iterator funcType=functionClasses.begin(); 
         ...
         int counter =0;
         //	while (prob<0.05){
         while (prob<0.05 && order<ORDER){ //FIXME
         ...
         if (saveMultiPdf){
            ...
            while (prob<upperEnvThreshold && order<ORDER){
               RooAbsPdf *bkgPdf = getPdf(pdfsModel,*funcType,order,Form("env_pdf_%d_%s",(cat+catOffset),ext.c_str()));
               if (!bkgPdf ){
                  // assume this order is not allowed
                  if (order>3) { std::cout << " [WARNING] could not add ] " << std::endl; break ;}
                  order++;
               }
               else if (order<ORDER){
                  //RooFitResult *fitRes;

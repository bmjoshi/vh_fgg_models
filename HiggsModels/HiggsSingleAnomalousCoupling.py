...
#        self.modelBuilder.factory_('expr::muVc_ZZ("@2/(@0**4+@1**4*{sigma3_VBF_ZZ}/{sigma1_VBF_ZZ})", a1,a3,muV)'.format(**xsecs))
    ...
    def getYieldScale(self,bin,process):
        ...
        if process in ["wh_ALT_0PM_%s_hgg"%y for y in years]: # should separate W and Z
            scale = 'smCoupling_WH'
        if process in ["zh_ALT_0PM_%s_hgg"%y for y in years]: # not yet there
            scale = 'smCoupling_ZH'
        if process in ["qqH_%s_%s_hgg"%(self.altSignal,y) for y in years]:
            scale = 'bsmCoupling_VBF'
        if process in ["wh_%s_%s_hgg"%(self.altSignal,y) for y in years]: # not yet considered
            scale = 'bsmCoupling_WH'
        if process in ["zh_%s_%s_hgg"%(self.altSignal,y) for y in years]: # not yet considered
            scale = 'bsmCoupling_ZH'
        if process in ["qqH_%sf05_%s_hgg"%(self.altSignal,y) for y in years]:
            scale = 'intCoupling_VBF'
        if process in ["zh_%sf05_%s_hgg"%(self.altSignal,y) for y in years]: # not yet considered
            scale = 'intCoupling_ZH'
        if process in ["wh_%sf05_%s_hgg"%(self.altSignal,y) for y in years]: # not yet considered 
